               ...
               size_t *pSize)
{
    ...
    if (fgetpos(pFile->pHandle, &position) != 0) {
        fprintf(stderr, "could not retrieve the current cursor position for "
                        "the file '%s'\n", pFile->pPath);
        out = DK_ERROR;
        goto exit;
    }
    ...
    if (fseek(pFile->pHandle, 0, SEEK_END) != 0) {
        fprintf(stderr, "could not reach the end of the file '%s'\n",
                pFile->pPath);
        out = DK_ERROR;
        goto exit;
    }
    ...
    if (size == EOF) {
        fprintf(stderr, "could not retrieve the size of the file '%s'\n",
                pFile->pPath);
        out = DK_ERROR;
...
position_restoration:
    ...
    if (fsetpos(pFile->pHandle, &position) != 0) {
        fprintf(stderr, "could not restore the cursor position for the file "
                        "'%s'\n", pFile->pPath);
        out = DK_ERROR;
    }
